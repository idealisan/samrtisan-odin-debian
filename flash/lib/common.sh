...
# 首启包含在线扩容（2 GiB → 约 111 GiB），reports/018 §三③ 实测要等 2-4 分钟
# （手册里写的是 sleep 150）。之前给 90/150 秒，实测必然超时 —— 扩容还没跑完，
# usb0 还没起来，脚本就误判失败往下走了。改成 240 秒留足余量。
T_USBNET=${T_USBNET:-240}       # 重启后等 PC 上出现 USB 网卡（首启含 resize2fs，慢）
T_SSH=${T_SSH:-240}             # 等 SSH 端口
