...
### 3.5 开发中的版本：正式号后面加简述后缀

**在一个目标版本（比如 `v0.9.4`）真正成功之前，中间所有尝试都用带简述后缀的号：**

```
v0.9.4-<简述>      例：v0.9.4-lk2nd-reboot、v0.9.4-makefile、v0.9.4-wifi-fix
```

规则：

- **只有最终验证通过、要对外正式发布的那一次，才用干净的三段式 `v0.9.4`**；
- 中间版本一律是 **Pre-release**，用来跑 CI、下载、真机验证；
- 后缀用**小写 ASCII + 连字符**，取这次改动的核心词，能一眼看出"这一版在试什么"；
  不要写 `fix1` / `try2` 这类无信息量的流水号，也不要用时间戳；
- 一次尝试里若同时改了多处，取最主要的那一项命名，细节写在 Release 说明里；
- 发现中间版本有问题 → 修 → **再开一个新的后缀号**，同样不回头改；
- 正式版 `v0.9.4` 的说明里要交代「经过哪几个后缀版本才收敛」以及各自踩了什么坑。

后缀示例（取自本项目实际会遇到的改动类型）：

| 后缀 | 含义 |
|---|---|
| `-lk2nd-reboot` | 修 lk2nd 反复重启、需人工按电源键 |
| `-makefile` | 构建从纯脚本改为 Makefile |
| `-modem-fw` | 蜂窝固件供给 |
| `-panel-detect` | 面板识别相关 |

这样做的好处：`v0.9.4` 这个名字**永远只对应那个验证通过的、能刷进真机的版本**，
不会有人拿到一个"v0.9.4 但是坏的"。而中间的失败尝试也全部留痕，且从名字就能
看出当时在试什么，回溯比时间戳友好得多。
