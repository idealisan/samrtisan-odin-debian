...
[Service]
Type=oneshot
# 这里**故意不写** RemainAfterExit=yes。原因（真机实测）：写了之后 unit 执行完
# 永不转 inactive，而看门狗用 OnUnitActiveSec=30s，它依赖 unit 的激活周期排下一跳；
# unit 一直 active 时 systemd 252（Debian 12）不会重新排期 ⇒ timer 触发一次就再也不
# 触发（实测：LastTriggerUSec 停在开机后 25.8s，TimersMonotonic 里 next_elapse=0）。
...
OnBootSec=10s
OnUnitActiveSec=30s
# 日历式兜底：OnUnitActiveSec 依赖 unit 的激活周期，一旦 unit 状态异常就不会重排
# ⇒ 看门狗静默失效。加一条 OnCalendar 保证无论如何都会周期性重排，宁可多跑一次也别漏。
OnCalendar=*:0/1
AccuracySec=1s
...
if [ ! -L "$R/etc/systemd/system/odin-usb-gadget.service" ]; then
	...
	# 注意：**不要**在这里 enable odin-dnsmasq@usb0.service。
	# 它不参与开机事务，只在 usb0 就绪后由 odin-usb-role.sh 按需 systemctl start。
	# 若在 multi-user.target.wants 里放符号链接，开机时 usb0 还没成为 systemd device
	# unit，该 unit 会一直 starting，进而把依赖它的服务拖死（真机实测开机卡死）。
	say "usb-role: service+watchdog timer 已就位并启用（dnsmasq 由脚本按需启动）"
else
